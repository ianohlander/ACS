...
export function createArtifactIntegrityReport(
  forkableArtifact: ForkableProjectArtifact,
  standaloneArtifact: StandalonePlayableArtifact,
  ...
  const checks: ArtifactIntegrityCheck[] = [
    createIntegrityCheck(
      "release-metadata-match",
      forkableRelease.id === standaloneRelease.id
        && forkableRelease.label === standaloneRelease.label
        && forkableRelease.version === standaloneRelease.version,
      `Forkable release ${forkableRelease.label} (${forkableRelease.id}) and standalone release ${standaloneRelease.label} (${standaloneRelease.id}) should describe the same immutable release.`
    ),
    createIntegrityCheck(
      "shared-handoff-parity",
      releaseHandoffManifestsMatch(forkableArtifact.releaseHandoffManifest, standaloneArtifact.releaseHandoffManifest),
      "Forkable and standalone exports should produce the same shared release handoff manifest."
    ),
    createIntegrityCheck(
      "forkable-package-files",
      forkablePackagePaths.has(forkableArtifact.projectManifest.handoff.packagedArtifactFileName)
        && forkablePackagePaths.has("project-manifest.json")
        && forkablePackagePaths.has(forkableArtifact.releaseHandoffManifest.handoff.packagedFileName),
      "Forkable package should include its packaged project artifact, project-manifest.json, and RELEASE-HANDOFF.json."
    ),
    createIntegrityCheck(
      "standalone-bundle-files",
      standaloneBundlePaths.has(standaloneArtifact.distributionManifest.package.entryFile)
        && standaloneBundlePaths.has("bundle/adventure-package.json")
        && standaloneBundlePaths.has("bundle/distribution-manifest.json")
        && standaloneBundlePaths.has(standaloneArtifact.releaseHandoffManifest.handoff.packagedFileName),
      "Standalone bundle should include its entry file, bundle/adventure-package.json, bundle/distribution-manifest.json, and RELEASE-HANDOFF.json."
    ),
    createIntegrityCheck(
      "archive-name-parity",
      sharedHandoff.artifacts.forkableProject.recommendedArchiveFileName === forkableArtifact.projectManifest.handoff.recommendedArchiveFileName
        && sharedHandoff.artifacts.standalonePlayable.recommendedArchiveFileName === standaloneArtifact.distributionManifest.handoff.recommendedArchiveFileName,
      "Shared handoff archive names should match the forkable and standalone package manifests."
    ),
    createIntegrityCheck(
      "release-notes-path-parity",
      sharedHandoff.artifacts.forkableProject.releaseNotesText === forkableArtifact.projectManifest.handoff.releaseNotesText
        && sharedHandoff.artifacts.standalonePlayable.releaseNotesText === standaloneArtifact.distributionManifest.handoff.releaseNotesText,
      "Shared handoff release-notes paths should match the packaged forkable and standalone release-notes files."
    )
  ];
...
function createIntegrityCheck(name: string, passes: boolean, details: string): ArtifactIntegrityCheck {
  return {
    name,
