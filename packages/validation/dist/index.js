...
function validateMediaAndSoundCues(pkg) {
    const issues = [];
...
function validateTriggerAction(pkg, mapsById, dialogueIds, itemIds, questIds, mediaCueIds, soundCueIds, trigger, triggerIndex, action, actionIndex) {
...
function validateTeleportAction(mapsById, trigger, triggerIndex, action, actionIndex) {
...
function validateQuestStageAction(pkg, questIds, trigger, triggerIndex, action, actionIndex) {
...
function questObjectiveCount(quest) {
    return quest.objectives.length || quest.stages?.length || 0;
}
function validateChangeTileAction(pkg, mapsById, trigger, triggerIndex, action, actionIndex) {
    const issues = [];
...
function unknownActionIssue(code, trigger, triggerIndex, actionIndex, message) {
