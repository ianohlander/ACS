...
Milestone 31N adds `AiGenerationSessionImportDossierArchive`, which turns the dossier bundle into one archive-ready import artifact with declared entry metadata and archive bytes. Later editor/API import-review surfaces can now export or persist ZIP-ready reviewed import dossiers without reimplementing package rules in UI code.
